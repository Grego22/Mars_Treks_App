...
(function() {
	...
	EkkoLightbox = function(element, options) {
		...
		if (!this.options.remote) {
			...
			if (this.gallery) {
				this.gallery_items = this.$element
					...
					.first()
					.find(
						'*[data-toggle="lightbox"][data-gallery="' + this.gallery + '"]'
					);
		...
		this.modal
			...
			.on("shown.bs.modal", this.options.onShown.bind(this))
			.on("hide.bs.modal", this.options.onHide.bind(this))
			.on("hidden.bs.modal", this.options.onHidden.bind(this))
			.modal("show", options);
		return this.modal;
	};
	...
	EkkoLightbox.prototype = {
		...
		checkImageDimensions: function(img) {
			...
			w = $(window);
			if (
				img.width + (this.padding.left + this.padding.right + 20) >
				w.width()
			) {
				return (img.width =
					w.width() - (this.padding.left + this.padding.right + 20));
			}
		}
	};
