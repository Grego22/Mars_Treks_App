/* jquery.nicescroll 3.1.0 InuYaksa*2012 MIT http://areaaperta.com/nicescroll */ (function(
	e
) {
	var r = false,
		w = false,
		...
		E = function() {
			...
			if (c.hastransform)
				(e.style[c.trstyle] =
					"translate3d(1px,2px,3px)"), (c.hastranslate3d = /translate3d/.test(
					e.style[c.trstyle]
				));
			c.transitionstyle = false;
			c.prefixstyle = "";
		...
		F = function(j, c) {
			...
			function i(d, c) {
				...
				"wheelDeltaY" in d
					? (
							...
							(f = Math.floor(d.wheelDeltaX * e)),
							(g = Math.floor(d.wheelDeltaY * e))
						)
			...
			if (g.cantouch && g.ischrome && !g.isios && !g.isandroid)
				(this.istouchcapable = true), (g.cantouch = false);
			if (g.cantouch && g.ismozilla && !g.isios)
				(this.istouchcapable = true), (g.cantouch = false);
			this.delayed = function(d, c, f, g) {
				...
				if (e && e.last + f > h && !e.tt)
					...
					(b.delaylist[d] = { last: h, tt: 0 }), setTimeout(function() {
						c.call();
					}, 0);
			};
			...
			this.getTarget = function(b) {
				return !b
					? false
					: b.target ? b.target : b.srcElement ? b.srcElement : false;
			};
			this.hasParent = function(b, c) {
				if (!b) return false;
				for (var f = b.target || b.srcElement || b || false; f && f.id != c; )
					f = f.parentNode || false;
				return f !== false;
			};
			...
			this.updateScrollBar = function(d) {
				if (b.ishwscroll)
					b.rail.css({ height: b.win.innerHeight() }), b.railh &&
						b.railh.css({ width: b.win.innerWidth() });
				else {
					var c = b.getOffset(),
						f = c.top,
						g = c.left;
					...
					if (b.railh && !b.locked)
						(f = c.top), (g = c.left), (d = b.railh.align
							? f +
								h(b.win, "border-top-width", true) +
								b.win.innerHeight() -
								b.railh.height
							: f + h(b.win, "border-top-width", true)), (g += h(
							b.win,
							"border-left-width"
						)), b.railh.css({
							top: d,
			...
			this.init = function() {
				...
				if (!b.ispage || (!g.cantouch && !g.isieold && !g.isie9mobile)) {
					...
					var h,
						...
						y;
					for (y in j)
						(i = j[y]), (h = b.opt.railpadding[i])
							? f.css("padding-" + i, h + "px")
					...
					if (b.ispage)
						f.css({ position: "fixed", top: "0px", height: "100%" }), f.align
							? f.css({ right: "0px" })
							: f.css({ left: "0px" }), b.body.append(f), b.railh &&
							(
						...
						if (b.ishwscroll)
							b.win.css("position") == "static" &&
								b.css(b.win, { position: "relative" }), (d =
								b.win[0].nodeName == "HTML" ? b.body : b.win), b.zoom &&
								(
									b.zoom.css({
										position: "absolute",
										top: 1,
									...
									d.append(b.zoom)
								), f.css({ position: "absolute", top: 0 }), f.align
								? f.css({ right: 0 })
								: f.css({ left: 0 }), d.append(f), k &&
								(
					...
					if (g.isie9mobile)
						(b.scrollmom = new z(b)), (b.onmangotouch = function() {
							...
							if (Math.round(Math.sqrt(Math.pow(l, 2) + Math.pow(f, 2))) != 0) {
								...
								if (
									...
									b.scrollmom.stop(), b.scrollmom.reset(
										c,
										d
									), (b.mangotouch.sy = d), (b.mangotouch.ly = d), (b.mangotouch.sx = c), (b.mangotouch.lx = c), (b.mangotouch.dry = g), (b.mangotouch.drx = e), (b.mangotouch.tm = h);
								else if (
									(
										b.scrollmom.stop(),
										b.scrollmom.update(
						...
						}), (f = b.getScrollTop()), (k = b.getScrollLeft()), (b.mangotouch = {
							sy: f,
							ly: f,
							dry: 0,
							sx: k,
						...
						if (
							...
							b.ontouchstart = function(d) {
								...
								if (!b.locked) {
									...
									if (!("clientX" in d) && "changedTouches" in d)
										(d.clientX = d.changedTouches[0].clientX), (d.clientY =
							...
							b.ontouchmove = function(d, c) {
								...
								if (b.rail.drag && b.rail.drag.pt == 2) {
									if (g.cantouch && typeof d.original == "undefined")
										return true;
									b.hasmoving = true;
									if (b.preventclick && !b.preventclick.click)
										(b.preventclick.click =
											b.preventclick.tg.onclick ||
											false), (b.preventclick.tg.onclick = b.onpreventclick);
						...
						g.cantouch || b.opt.touchbehavior
							? (
									(b.onpreventclick = function(d) {
										if (b.preventclick)
											return (b.preventclick.tg.onclick =
												b.preventclick.click), (b.preventclick = false), b.cancelEvent(
												d
											);
									}),
							...
							: (
									(b.onmousedown = function(d, c) {
										if (!(b.rail.drag && b.rail.drag.pt != 1)) {
											...
											if (b.isiframe && !g.hasmousecapture)
												(b.saved.csspointerevents = b.doc.css(
													"pointer-events"
						...
						b.railh &&
							(
								...
								b.bind(b.cursorh, "mouseup", function(d) {
									if (!(b.rail.drag && b.rail.drag.pt == 2))
										return (b.rail.drag = false), (b.hasmoving = false), b.hideCursor(), g.hasmousecapture &&
											document.releaseCapture(), b.cancelEvent(d);
								})
							);
						...
						b.opt.enablemousewheel &&
							(
								b.isiframe ||
									b.bind(
										g.isie && b.ispage ? document : b.docscroll,
										"mousewheel",
										b.onmousewheel
									),
					...
					b.onkeypress = function(d) {
						...
						if (
							...
							switch (c) {
								...
								case 63232:
									b.railh &&
										(f ? b.doScrollLeft(0) : b.doScrollLeftBy(72), (l = true));
									break;
								case 39:
								case 63234:
									b.railh &&
					...
					var A = function() {
						...
						if (b.iframexd)
							return "console" in window &&
								console.log("NiceScroll error: policy restriced iframe"), true;
						b.forcescreen = true;
						if (b.isiframe)
							(b.iframe = {
								doc: e(d),
						...
						b.opt.enablekeyboard &&
							b.bind(d, g.isopera ? "keypress" : "keydown", b.onkeypress);
						if (g.cantouch || b.opt.touchbehavior)
							b.bind(
								...
								b.css(e(d.body), { cursor: g.cursorgrabvalue });
				...
				if (!b.cursortimeout && b.rail && b.autohidedom)
					b.cursortimeout = setTimeout(function() {
						if (!b.rail.active || !b.showonmouseevent)
							b.autohidedom
								.stop()
								.animate({ opacity: b.opt.cursoropacitymin }), b.zoom &&
								b.zoom
									.stop()
									.animate({
			...
			this._bind = function(d, c, f, g) {
				...
				d.addEventListener
					? d.addEventListener(c, f, g || false)
					: d.attachEvent ? d.attachEvent("on" + c, f) : (d["on" + c] = f);
			};
			this.bind = function(d, c, f, e) {
				...
				h.addEventListener
					? (
								...
								b._bind(
									...
									function(b) {
										if (b.touches) {
											...
											(d = b.changedTouches[0]), (d.original = b), f.call(
												this,
												d
											);
									},
							...
							b._bind(h, c, f, e || false),
							c == "mousewheel" && b._bind(h, "DOMMouseScroll", f, e || false),
							g.cantouch &&
								c == "mouseup" &&
								b._bind(h, "touchcancel", f, e || false)
						)
					: b._bind(h, c, function(d) {
							if ((d = d || window.event || false) && d.srcElement)
								d.target = d.srcElement;
							return f.call(h, d) === false || e === false
								? b.cancelEvent(d)
								: true;
						});
			};
			this._unbind = function(b, c, f, g) {
				b.removeEventListener
					...
					: b.detachEvent ? b.detachEvent("on" + c, f) : (b["on" + c] = false);
			};
			...
			this.showRail = function() {
				if (b.page.maxh != 0 && (b.ispage || b.win.css("display") != "none"))
					(b.visibility = true), (b.rail.visibility = true), b.rail.css(
						"display",
						"block"
					);
				return b;
					...
					? (
							(this.prepareTransition = function(c, e) {
								...
								if (!b.lasttransitionstyle || b.lasttransitionstyle != h)
									(b.lasttransitionstyle = h), b.doc.css(g.transitionstyle, h);
								return f;
							}),
									...
									if (i > 0) {
										...
										if (g.transitionend) {
											if (!b.scrollendtrapped)
												(b.scrollendtrapped = true), b.bind(
													b.doc,
													g.transitionend,
													b.onScrollEnd,
													false
			...
			this.doScrollBy = function(c, e) {
				...
				else if (f > b.page.maxh && py >= b.page.maxh)
			...
			this.doZoomIn = function(c) {
				if (!b.zoomactive) {
					...
					if (g.isios4)
						(b.zoomrestore.scrollTop = e(window).scrollTop()), e(
							window
						).scrollTop(0);
					b.win.css({
			...
			this.doZoomOut = function(c) {
				if (b.zoomactive)
					return (b.zoomactive = false), b.win.css("margin", ""), b.win.css(
						b.zoomrestore.style
					), g.isios4 &&
						e(window).scrollTop(b.zoomrestore.scrollTop), b.rail.css({
						"z-index": b.ispage ? b.opt.zindex : b.opt.zindex + 2
					}), b.zoom.css({
						"z-index": b.opt.zindex
					}), (b.zoomrestore = false), b.zoom.css(
						"backgroundPosition",
						"0px 0px"
					), b.onResize(), b.onzoomout && b.onzoomout.call(b), b.cancelEvent(c);
			};
					...
					var d = c.lastscrollx,
						...
						f = function() {
							...
							c.nc.synched("domomentum2d", function() {
								if (c.speedx)
									c.nc.getScrollLeft() != c.chkx &&
										c.stop(), (c.chkx = d), c.nc.setScrollLeft(d);
								if (c.speedy)
									c.nc.getScrollTop() != c.chky &&
										c.stop(), (c.chky = l), c.nc.setScrollTop(l);
								c.timer || (c.nc.hideCursor(), c.doSnapy(d, l));
							});
							c.demulxy < 1
								? (c.timer = setTimeout(f, o))
	...
	e.fn.niceScroll = function(j, c) {
		...
		this.each(function() {
			...
			if (!b)
				(c.doc = i ? e(this) : c.doc), (b = new F(c, e(this))), e(this).data(
					"__nicescroll",
					b
				);
			h.push(b);
