...
(function(e, t) {
	...
	function f(e, t, n) {
		e.stop();
		e
			.find(".tp-thumbcontainer")
			.animate({ left: t + "px" }, { duration: n, queue: false });
	}
	function l(t, n) {
	...
	function S(e, n) {
		...
		if (a.data("lazyload") != t && a.data("lazydone") != 1) {
			a.attr("src", u.find(".defaultimg").data("lazyload")), a.data(
				"lazydone",
				1
			);
			a.data("orgw", 0);
	...
	function x(n, r) {
		...
		if (l == 12 || l == 13 || l == 14 || l == 15) {
			...
			if (l == 12)
				...
				if (r.ie9) x.transition({ left: 0 - E + "px" }, 0);
				else x.transition({ left: 0 - E + "px", rotate: r.rotate }, 0);
			else if (l == 13)
				if (r.ie9) x.transition({ top: S + "px" }, 0);
				else x.transition({ top: S + "px", rotate: r.rotate }, 0);
			else if (l == 14)
				if (r.ie9) x.transition({ top: 0 - S + "px" }, 0);
				else x.transition({ top: 0 - S + "px", rotate: r.rotate }, 0);
			x.transition(
		...
		if (l == 16) {
			...
			i
				...
				.css({ position: "absolute", top: 0 - C - k + "px", left: "0px" });
			if (!e.support.transition) {
				i
					.find(".tp-half-one")
					.animate(
						{ opacity: 0, top: 0 - S / 2 + "px" },
				...
				i
					.find(".tp-half-one")
					.transition(
						{ opacity: 1, scale: O, rotate: L, y: 0 - S / 1.4 + "px" },
						800,
						"in"
					);
				i
					.find(".tp-half-two")
					.transition(
						{ opacity: 1, scale: _, rotate: A, y: 0 + S / 1.4 + "px" },
						800,
						"in"
					);
				if (i.html() != null)
					o
						.transition(
							{ scale: 0.8, x: r.width * 0.1, y: S * 0.1, rotate: L },
							0
						)
							...
							if (!n.hasClass("HasListener")) {
								try {
									...
									if (f.data("autoplay") == true)
										i = new YT.Player(c, {
											events: { onStateChange: N, onReady: C }
										});
									else i = new YT.Player(c, { events: { onStateChange: N } });
				...
				if (!o.hasClass("revslider-initialised")) {
					...
					if (s.shuffle == "on") {
						for (var w = 0; w < o.find(">ul:first-child >li").length; w++) {
							var E = Math.round(
								Math.random() * o.find(">ul:first-child >li").length
							);
							o
								.find(">ul:first-child >li:eq(" + E + ")")
								.prependTo(o.find(">ul:first-child"));
						}
					}
					s.slots = 4;
					s.act = -1;
					...
					if (s.shadow != 0) {
						o
							.parent()
							.append(
								'<div class="tp-bannershadow tp-shadow' + s.shadow + '"></div>'
							);
					...
					o.find(".tp-scrollbelowslider").on("click", function() {
						...
						e("body,html").animate(
							{
								scrollTop:
									o.offset().top + o.find(">ul >li").height() - t + "px"
							},
							{ duration: 400 }
						);
					});
