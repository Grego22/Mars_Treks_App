...
(function(a) {
	...
	var b = {};
	(a.respond = b), (b.update = function() {}), (b.mediaQueriesSupported =
			...
			for (var p in e)
				if (e.hasOwnProperty(p)) {
					var v = e[p],
						...
						z = null === x,
						A = "em";
					w && (w = parseFloat(w) * (w.indexOf(A) > -1 ? t || s() : 1)), x &&
						(x =
							parseFloat(x) *
							(x.indexOf(A) > -1 ? t || s() : 1)), (v.hasquery &&
						((y && z) || !(y || k >= w) || !(z || x >= k))) ||
			...
			for (var C in m)
				if (m.hasOwnProperty(C)) {
					var D = c.createElement("style"),
						E = m[C].join("\n");
					(D.type = "text/css"), (D.media = C), j.insertBefore(
						D,
						n.nextSibling
					), D.styleSheet
						? (D.styleSheet.cssText = E)
						: D.appendChild(c.createTextNode(E)), g.push(D);
				}
		},
		v = function(a, b) {
			var c = w();
